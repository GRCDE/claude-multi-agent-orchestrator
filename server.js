...
// ── Webhook testen ───────────────────────────────────────────
app.post('/api/test-webhook', authMiddleware, apiLimiter, async (req, res) => {
  const { url } = req.body;
...
// ── Webhook-System (Event-Hooks fuer externe Integrationen) ───
      ...
      try {
        ...
        setTimeout(async () => {
          try {
            ...
            logger.warn('Webhook Retry fehlgeschlagen', { webhookId: hook.id, event: eventName, error: retryErr.message });
          }
        }, WEBHOOK_RETRY_DELAY);
      }
    })().catch(e => {
      logger.warn('Webhook-Dispatch Fehler', { webhookId: hook.id, event: eventName, error: e.message });
    });
  }
}
...
orchestrator.on('agent', (data) => {
  if (data.agent && data.agent.status === 'done') {
    dispatchWebhooks('agent-done', data);
  }
  if (data.agent && data.agent.status === 'error') {
    dispatchWebhooks('agent-error', data);
  }
});

orchestrator.on('coordinator', (data) => {
  if (data.tasks && data.tasks.length > 0 && data.phase === 'plan_created') {
...
app.post('/api/webhooks', authMiddleware, apiLimiter, async (req, res) => {
...
app.get('/api/webhooks', authMiddleware, apiReadLimiter, async (req, res) => {
  try {
    const hooks = await loadWebhooks();
...
app.delete('/api/webhooks/:id', authMiddleware, apiLimiter, async (req, res) => {
...
app.post('/api/webhooks/:id/test', authMiddleware, apiLimiter, async (req, res) => {
  const whId = req.params.id;
  try {
    const hooks = await loadWebhooks();
...
app.post('/api/webhooks/:id/toggle', authMiddleware, apiLimiter, async (req, res) => {
  const whId = req.params.id;
  try {
    const hooks = await loadWebhooks();
    const hook = hooks.find(h => h.id === whId);
...
app.get('/api/docs', (req, res) => {
  ...
  const html = '<!DOCTYPE html><html><head><meta charset="utf-8"><title>' + docs.title + '</title><style>'
    ...
    + '<div class="formats"><a href="/api/docs/json">JSON</a><a href="/api/docs/openapi">OpenAPI 3.0</a><a href="/api/docs/markdown">Markdown</a></div>'
    + categoriesHtml
    + '</div></body></html>';
